...
//		                  "4564806ed7130cebb6ef39e32f8e2fbbf63d4777","795d1da1a349a3d9466d1902d27597bafb11e27f",
...
//
//		            oa.getOAuthAccessToken(oauth_token, oauth_token_secret, accessTokenCallback);
//		    }
//		});
//				
...
//		            console.log('error: ' + JSON.stringify(error));
//		        } else {		
//		            console.log('getting a list of tracks by nicky romero');
//					var url = 'https://oauth-api.beatport.com/catalog/3/search/?query=Symphonica&facets=artistName:Nicky Romero',
//					    access_token = "1a2b472e835f9fd11d0d2cdb397d35450750f427",
//					    access_token_secret = "623c75c0e62e1fa3d58c8a2f6c16ab2cc206198a";
//					
...
//		
	...
	searchMuzofon: function (searchterm) {
	        if (this.isSimulation) {
	                return undefined;
	        }
	        this.unblock;
	        ...
	        while (retries <= maxretries){
	                try {
	                        var result = Meteor.http.get("http://muzofon.com/search/" + searchterm, {
	                                timeout: 5000
	                        });
	                        if (result && result.statusCode && result.statusCode === 200) {
	                                ...
	                                retries++;
	                                console.log("Error in muzofon communication, retrying");
	                        }
	                }
	                catch (e) {
	                        retries++;
	...
	searchMyFreeMP3: function (searchterm) {
		if (this.isSimulation) {
			return undefined;
		}
		this.unblock;
		...
		while (retries <= maxretries){
			try {
				var result = HTTP.get("http://myfreemp3.eu/music/" + searchterm, {
					timeout: 5000
				});
				if (result && result.statusCode && result.statusCode === 200) {
					return result;
					...
					retries++;
					console.log("Error in myfreemp3.eu communication, retrying");
				}
			}
			catch (e) {
				retries++;
	...
	searchMP3Monkey: function (searchterm) {
		if (this.isSimulation) {
			return undefined;
		}
		this.unblock;
		...
		while (retries <= maxretries){
			try {
				var result = HTTP.get("http://mp3monkey.net/mp3/" + searchterm.replace(" ","_") + ".html", {
					timeout: 5000
				});
				if (result && result.statusCode && result.statusCode === 200) {
					return result;
					...
					retries++;
					console.log("Error in mp3monkey.net communication, retrying");
				}
			}
			catch (e) {
				retries++;
