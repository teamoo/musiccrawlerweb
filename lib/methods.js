...
Meteor.methods({
	...
	markLinksAsDownloadedByDate: function (sel_date, filter_sites, include_vk) {		
		if (include_vk)		
			query = {
				date_published: {
					$gte: sel_date
		...
		while (retries <= maxretries){
			try {
				...
				if (result && result.statusCode && result.statusCode === 200) {
					...
					retries++;
					console.log("Error in muzon.ws communication, retrying");
				}
			}
			catch (e) {
				retries++;
	...
	createLink: function (linkurl, stream_url, name, aid, oid) {
		this.unblock();
		if (this.isSimulation) {
			return undefined;
		}
