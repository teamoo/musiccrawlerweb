...
Meteor.methods({
	...
	createLink: function (linkurl, stream_url) {
		...
		if (result && result.statusCode && result.statusCode === 200) {
			if (result.content) {
				var freshlink = result.data;
				if (freshlink.status == "off" && freshlink.url.indexOf("muzon.ws") === -1) throw new Meteor.Error(404, "Link ist offline", "Der Link " + linkurl + " ist offline und kann daher von niemandem heruntergeladen werden. Der Link wird deshalb nicht in die Sammlung aufgenommen.");
