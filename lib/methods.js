...
Meteor.methods({
	getSuggestionsForSearchTerm: function(searchterm) {
		//TODO auto-suggest implementieren
		var result = ["toll","besser","unglaublich","treffer","noch ein treffer"];
		return result;
	},
	...
	searchMuzon: function (searchterm) {
		...
		var result = Meteor.http.get("http://muzon.ws/music/" + searchterm, {
			timeout: 5000, encoding: 'base64'
		});
		if (result && result.statusCode === 200) {
			return result;
			//TODO: iconv zum Laufen bekommen, damit das encoding machen
			/*
			    var require = __meteor_bootstrap__.require;
				var Iconv = require('iconv').Iconv;
				var iconv = new Iconv('ISO-8859-1', 'UTF-8//TRANSLIT//IGNORE');
		...
		if (alink)
		{
			...
			if (result && result.statusCode === 200) {
				if (result.content) {
					...
					if (freshlink.name && freshlink.size != undefined && freshlink.status && freshlink.hoster) {
						var ret = Links.update({
							_id: linkid
						}, {
							$set: {
								name: freshlink.name,
								hoster: freshlink.hoster,
								size: freshlink.size,
