...
AdminNotifications = new Meteor.Collection("admin_notifications", {
	idGeneration: 'MONGO'
});

AdminStatistics = new Meteor.Collection("admin_statistics", {
	idGeneration: 'MONGO'
});

UnreadCounts = new Meteor.Collection("unreadcounts");
...
Links.allow({
	...
	update: function (userId, link, fields, modifier) {
		...
		if (fields.indexOf("likers") !== -1) return !(_.contains(link.likers, Meteor.userId()));
		return true;
	},
