...
Meteor.methods({
	...
	checkSite: function (siteurl) {
		if (this.isSimulation)
		{
	   	...
	   	//return("SITE: " + siteurl + " RE: " + resiteurl);
	   	
	   	if (Sites.findOne({url : {$regex : resiteurl, $options: 'i' }}) != undefined)
	   	{
	   		...
	   		{	
		   		...
		   		if (m == null) {		   			
		   			...
		   			newSite.name = siteurl.replace("http://","").replace("www.","");
		   			...
		   			return new Meteor.Error(415,"unbekannter Seitentyp","Für die Seite " + siteurl + " konnte kein RSS-Feed gefunden werden. Die Seite wird manuell überprüft und dann durchsucht."), Sites.insert(newSite);
		   		}
		   		else {
		   			...
		   			if (Sites.findOne({feedurl : {$regex : renewfeedurl, $options: 'i' }}) != undefined) 
		   			{
		   				throw new Meteor.Error(409,"Seite bereits vorhanden","Die Seite " + siteurl + " ist bereits im System vorhanden")
		   				return undefined;
		   			}
	   		...
	   		else throw new Meteor.Error(404,"Seite konnte nicht geladen werden","Die Seite " + siteurl + " konnte nicht geladen werden.");
	   		return undefined;
	   	}
	},
	
	//TODO: funktioniert noch nicht...
	createSite: function (siteObject) {
			if (this.isSimulation)
			{
				Sites.insert({name: siteObject.name, url: siteObject.url, feedurl: siteObject.feedurl, domain: siteObject.domain, active: siteObject.active, last_crawled : null, type: siteObject.type, creator : this.user().profile.id });
			};
