...
//	},
		...
		if (infoObject.user && infoObject.user.services && infoObject.user.services.facebook && infoObject.user.services.facebook.accessToken)
		{
			try {
				...
				if (result && result.data && result.data.data && result.data.data.url)
				{
					...
					winston.log("Profilbild erfolgreich aktualisiert für Benutzer " + infoObject.user._id,{action:"updateFacebookPictureForUser"});
				}
				else
					winston.warn("Profilbild konnte nicht von Facebook abgerufen werden.",{action:"updateFacebookPictureForUser"})
			}
			catch (e) {
				winston.warn("Profilbild konnte nicht von Facebook abgerufen werden.",{action:"updateFacebookPictureForUser"})
			}
	...
	updateFacebookTokensForUser: function (infoObject) {
		...
		if (infoObject.user && infoObject.user.services && infoObject.user.services.facebook) {		
			...
			winston.error("Token konnten nicht aktualisiert werden: Kein gültiges Benutzerobjekt.",{title:"Fehler bei Token-Update.",action:"updateFacebookTokensForUser"})
	},
	...
	searchExternalService: function(searchterm, servicename) {
		...
		switch (servicename) {
			...
			case "mp3monkey":
				...
				break;
			default:
				winston.error("Unbekannter externer Suchanbieter " + servicename,{title:"unbekannter externer Suchanbieter.",action:"searchExternalService"});
	...
	refreshLink: function (linkid) {
		...
		if (alink) {
			try {
				...
				winston.error("Fehler beim aktualisieren des Links " + alink.url + " : ungültige Antwort vom Server",{title:"Fehler in LinkChecker Kommunikation",action:"refreshLink"});
			...
			if (m == null) {
				...
				if (ok) {
					...
					try {
						...
						winston.error("Fehler beim Senden der Benachrichtigungs-eMail zur Seitenerstellung:" + error.details, {title: "eMail Senden fehlgeschlagen.",action:"createSite", object: newSite})
					}
