...
Meteor.methods({
	...
	getSuggestionsForEmail: function(emailaddress) {		
		var ret = Meteor.users.find({'services.facebook.email': {$regex: emailaddress, $options:'i'}},{limit:5,fields:{services:1}}).fetch();	
		return _.map(ret, function(item){ return _.pick(item.services.facebook, 'name', 'email') ; });
	},
		...
		if (alink)
		{
			...
			if (result && result.statusCode === 200) {
				if (result.content) {
					...
					if (freshlink) {
						if (freshlink.name && freshlink.name.indexOf("file") === -1 && alink.url.indexOf("/"+freshlink.name) === -1)
...
*/
    ...
    shareLinks: function (emailaddress, linkids) {
		...
		var thelinks = Links.find({_id: {$in: _.map(linkids, function(linkid){ return new Meteor.Collection.ObjectID(linkid); })}}).fetch();
		...
		if (thelinks && thelinks.length && sender && emailaddress)
		{
			...
			{
				var setid = Sets.insert({creator: Meteor.user().id, links: linkids, date_created: new Date()});
				...
				if (setid)
				{
					...
					for (link in thelinks) {
						...
						var sizeinMB = Math.round(link.size / 1048576);
						...
						datestring = link.date_published.getDate() + "." + link.date_published.getMonth()+1 + "." + link.date_published.getYear();
						
						links_table_string += "<tr mc:repeatable><td valign=\"top\" class=\"dataTableContent\" mc:edit=\"data_table_content00\">";
						links_table_string += link.name;
						...
						links_table_string += link.date_published;
					...
					try {
						Email.send({
							...
							"Hi,\n" + sender + " möchte dir folgende Links zeigen:\n" +
										 "<a href='" + Meteor.absoluteUrl("set/"+ setid) + "'>Links anzeigen</a>"			 
							*/
						});
					}
					catch(error) {
