...
Meteor.methods({
	createAdminNotification: function(action, object) {
		var notification = new Object();
		notification.date_created = new Date();
		notification.read_by = [];
		//TODO Notifikationsereignisse nutzen und ausweiten
		switch (action)
		{
			case "createSite":
				notification.action = action;
				notification.title = "Neue Seite hinzugefügt";
				notification.message = "Eine neue Seite '" + object.name + "' (URL: " + object.feedurl + ") wurde von " + Meteor.users.findOne({id: Meteor.user().id}).profile.name + " hinzugefügt."
				break;
			case "scheduleCleanup":
				notification.action = action;
				notification.title = "Datenbankbereinigung gestartet";
				notification.message = "Eine neue Datenbankbereinigung wurde von " + Meteor.users.findOne({id: Meteor.user().id}).profile.name + "mit JobID" + object.jobid + " gestartet."
				break;
		}
		
		AdminNotifications.insert(notification);
	},
		...
		while (retries <= maxretries){
		        try {
		                ...
		                if (result && result.statusCode && result.statusCode === 200) {
		                        ...
		                        retries++;
		                        console.log("Error in " + servicename +  " communication, retrying");
		                }
		        }
		        catch (e) {
		                retries++;
	...
	createSite: function (siteurl) {
		...
		if ((siteurl.indexOf("vk.com/") !== -1) || (siteurl.indexOf("vkontakte.ru/") !== -1)) {
			...
			if (ok) {
				createAdminNotification("createSite", newSite)
			}
			return ok;
	...
	scheduleCleanup: function () {
		...
		if (Meteor.user().admin === true)
		{
			...
			if (result && result.statusCode && result.statusCode === 200) {
				...
				createAdminNotification("scheduleCleanup", result.data);
