...
*/
    ...
    shareLink: function (emailaddress, linkid) {
        if (this.isSimulation) {
            return undefined;
        }
        ...
        var alink = Links.findOne({_id:linkid});
        var sender = Meteor.user().profile.name;
        
		if (alink && sender && emailaddress)
		{
			try {
				Email.send({
					to: emailaddress,
					from: "info@meteormusiccrawler.de",
					subject: "MusicCrawler Link: " + alink.name + " wurde für dich freigegeben",
					html: "Hi,\n" + sender + " möchte dir diesen Link zeigen:\n" +
								 "<a href='" + Meteor.absoluteUrl("links/"+ alink._id) + "'>" + alink.name + "</a>"
				});
			}
			catch(error) {
				console.log(error);
