...
Meteor.methods({
	...
	getSuggestionsForSearchTermV2: function (searchterm) {
		...
		MongoInternals.defaultRemoteCollectionDriver().mongo.db.executeDbCommand({"text":"links",search: searchterm_mod, limit:10, project:{_id:0, name:1}},
		//var results = Links.find({$text: {$search: searchterm_mod}}).limit(10).
			function (error,results){
				if (results && results.documents[0].ok === 1)
				{	
					var ret = results.documents[0].results;
					fut.return(_.uniq(_.map(_.pluck(ret, 'obj'), function (text) {
		...
		if (Meteor.user() && Meteor.user().services.facebook) {
			try {
				HTTP.call("GET", "https://graph.facebook.com/" + groupid + "/?fields=name&access_token=" + Meteor.user().services.facebook.accessToken, function (error, result) {
					if (error) 
						winston.log("Gruppenname konnte nicht abgerufen werden: " + error,{object: groupid, action:"updateFacebookGroupName"});
				...
				winston.log("Fehler beim Abfragen des Gruppennamen für die Gruppe mit ID " + groupid + " und AccessToken " + Meteor.user().services.facebook.accessToken + error,{object: groupid, action:"updateFacebookGroupName", title:"Gruppenname konnte nicht abgerufen werden."});
			}
		}
	},
	...
	checkJDOnlineStatus: function (options1) {
		...
		try {
			...
			winston.warn("Fehler beim Aktualisieren des JD-Remote-Status mit URL " + "http://" + options1['ip'] + ":" + options1['port'] + "/get/version" ,{action:"checkJDOnlineStatus"});
			return false;
		}
	},
	refreshLink: function (linkid) {
		...
		if (alink) {
			try {
				...
				winston.warn("Fehler beim aktualisieren des Links " + alink.url + " : ungültige Antwort vom Server",{action:"refreshLink"});
		...
		winston.error("Fehler beim aktualisieren des Links " + alink.url + " : Link nicht in der Datenbank gefunden",{action:"refreshLink"});
	...
	createLink: function (linkobject, linkurl, stream_url, name, aid, oid) {
		...
		if (linkurl.indexOf("beatport.com") != -1) {
			...
			if (ret) {
				winston.info("Eine neuer Link wurde hinzugefügt",{object: freshlink, action:"createLink",title:"Neue Seite hinzugefügt", ready_by: [], creator:Meteor.user().profile.name});
			}
			return ret;
		}
	...
	createSite: function (siteurl) {
		...
		if ((siteurl.indexOf("vk.com/") !== -1) || (siteurl.indexOf("vkontakte.ru/") !== -1)) {
			...
			if (ok) {
				winston.info("Eine neue Seite wurde hinzugefügt",{object: newSite, action:"createSite",title:"Neue Seite hinzugefügt", ready_by: [], creator:Meteor.user().profile.name});
			}
			return ok;
	...
	scheduleCleanup: function () {
		...
		if (Meteor.user().admin === true)
		{
			...
			if (result && result.statusCode && result.statusCode === 200) {
				...
				winston.info("Datenbank Bereinigungs-Lauf erfolgreich gestartet",{object: result.data, action: "scheduleCleanup", read_by: []})
