...
Meteor.methods({
	getLinkURLsByDate: function(sel_date, filter_sites) {
		return _.pluck(Links.find({
				date_published: {
					$gte: sel_date
				},
				status: {
	...
	markLinksAsDownloadedByDate: function (sel_date, filter_sites) {		
		query = {
			date_published: {
				$gte: sel_date
			},
	...
	markLinksAsDownloadedById: function (sel_links) {
		var samenames = _.pluck(Links.find({
				_id: {
					$in: _.map(sel_links, function (linkid) {
		...
		try {
			var res = Links.update(query, update, options);
		}
	...
	getSuggestionsForSearchTermV2: function (searchterm) {	
		...
		//searchterm_mod = searchterm_mod.trim().replace(".*","").replace(".*","").replace("(","").replace(")","");
		
		searchterm_mod = searchterm;
		...
		Meteor._RemoteCollectionDriver.mongo.db.db('musiccrawler').executeDbCommand({"text":"links",search: searchterm_mod, limit:10, project:{_id:0, name:1}},
			function (error,results){				
				if (results && results.documents[0].ok === 1)
				{	
					var ret = results.documents[0].results;
					fut.ret(_.uniq(_.map(_.pluck(ret, 'obj'), function (text) {
						if (text.name.length > 45) return text.name.substr(0, 44);
						return text.name;
					})));
				}
			}
	...
	updateFacebookGroupName: function (groupid) {
		...
		if (Meteor.user() && Meteor.user().services.facebook) {
			try {
				Meteor.http.call("GET", "https://graph.facebook.com/" + groupid + "/?fields=name&access_token=" + Meteor.user().services.facebook.accessToken, function (error, result) {
				...
				console.log("Fehler beim Abfragen des Gruppennamen für die Gruppe mit ID " + groupid + " und AccessToken " + Meteor.user().services.facebook.accessToken);
			}
		}
	},
	...
	searchMuzon: function (searchterm) {
		...
		try {
			...
			if (result && result.statusCode && result.statusCode === 200) {
				//var require = __meteor_bootstrap__.require;
				var Iconv = Npm.require('iconv').Iconv;
			...
			else console.log("Error in muzon.ws communication");
		}
		catch (e) {
			console.log("Error in muzon.ws communication");
			console.log(e);
		}
	},
	...
	checkJDOnlineStatus: function (options1) {
		...
		try {
			var result = Meteor.http.get("http://" + options1['ip'] + ":" + options1['port'] + "/get/version", {
				timeout: 4000
			});
	...
	refreshLink: function (linkid) {
		...
		if (alink) {
			try {
				...
				throw new Meteor.Error(500, "Fehler bei der Serverkommunikation", "Fehler beim aktualisieren des Links " + alink.url + " : ungültige Antwort vom Server");
			}
			...
			if (result && result.statusCode && result.statusCode === 200) {
				if (result.content) {
					...
					if (freshlink) {
						...
						if (freshlink.size != undefined) newsize = freshlink.size;
	...
	createLink: function (linkurl, stream_url) {
		...
		if (result && result.statusCode && result.statusCode === 200) {
			if (result.content) {
				var freshlink = result.data;
				if (freshlink.status == "off" && freshlink.url.indexOf("muzon.ws") === -1 && freshlink.url.indexOf("vk.me") !== -1) throw new Meteor.Error(404, "Link ist offline", "Der Link " + linkurl + " ist offline und kann daher von niemandem heruntergeladen werden. Der Link wird deshalb nicht in die Sammlung aufgenommen.");
	...
	createSite: function (siteurl) {
		...
		if (siteurl.indexOf("facebook.com/groups/") !== -1) {
			...
			if (newSite.groupid && newSite.groupid != "") {
				...
				if (ok) {
					try {
						var result = Meteor.http.get("https://graph.facebook.com/" + Meteor.user().id + "/permissions?access_token=" + Meteor.user().services.facebook.accessToken, {
							timeout: 4000
						});
					}
					catch (e) {
		...
		try {
			...
			console.log("Error receiving feed information from Google Feed API");
		}
