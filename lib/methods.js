...
Meteor.methods({
	...
	getSuggestionsForEmail: function(emailaddress) {		
		var ret = Meteor.users.find({$or: [{'services.facebook.email': {$regex: emailaddress, $options:'i'}}, {'services.facebook.name': {$regex: emailaddress, $options:'i'}}]},{limit:5,fields:{services:1}}).fetch();	
