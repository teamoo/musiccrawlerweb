...
Meteor.methods({
	...
	getSuggestions: function (searchterm) {
		...
		MongoInternals.defaultRemoteCollectionDriver().mongo.db.executeDbCommand({"text":"links",search: searchterm_mod, limit:10, sort:{textScore: -1, date_published: -1},project:{_id:0, name:1, size:1, date_published: 1}},
		...
		if (thelinks && thelinks.length && sender && emailaddress) {
			if (thelinks.length === 1) {
				try {
					...
					winston.error("Fehler beim Senden einer Link-Freigabe-Benachrichtigung: " + error.details,{action:"shareLinks", object: error})
