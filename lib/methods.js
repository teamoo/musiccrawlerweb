...
.methods({
    ...
    this.unblock();	
	var result = Meteor.http.post("http://" + Meteor.settings.linkbuilder.host + ":" + Meteor.settings.linkbuilder.port + "/musiccrawler/checkmusicdownloadlink",{data: {url: refreshurl}, timeout: 20000});
    ...
    return Links.find({date_discovered :{$gte : timespan} }).count();
		...
		//TODO testen
	scheduleCrawl : function(aFeedurl) {
		if (Meteor.user().admin === true)
		{
			this.unblock();
			Meteor.http.post("http://" + Meteor.settings.crawler.host + ":" + Meteor.settings.crawler.port + "/schedule.json", {data:{project:musiccrawler,spider:feedspider, feedurl:aFeedurl}, timeout: 3000})
	...
	//TODO testen
	cancelCrawl : function(aJobId) {	
		if (Meteor.user().admin === true)
		{
			this.unblock();
			Meteor.http.post("http://" + Meteor.settings.crawler.host + ":" + Meteor.settings.crawler.port + "/cancel.json", {data:{project:musiccrawler,job:aJobId}, timeout: 3000})
