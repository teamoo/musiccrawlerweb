...
.methods({
	...
	if (this.isSimulation) {
		...
		this.unblock();
		var result = Meteor.http.get(requeststring,{timeout: 4000});
	...
	var alink = Links.findOne({url: refreshurl});
	
    this.unblock();	
	var result = Meteor.http.post("http://" + Meteor.settings.linkbuilder.host + ":" + Meteor.settings.linkbuilder.port + "/musiccrawler/checkmusicdownloadlink",{data: {url: refreshurl, source: alink.source}, timeout: 30000});
    ...
    createLink : function(linkurl) {
		...
		var result = Meteor.http.post("http://" + Meteor.settings.linkbuilder.host + ":" + Meteor.settings.linkbuilder.port + "/musiccrawler/checkmusicdownloadlink",{data: {url: cleanlinkurl, source: userid, creator: userid}, timeout:30000});
	...
	if (this.isSimulation) {
	    var comment = {
		    creator : Meteor.user().profile.id,
		    message : aMessage,
    ...
    return Links.find({date_discovered :{$gte : timespan} }).count();
    },
	...
	scheduleCrawl : function(asiteId) {
		...
		if (Meteor.user().admin === true)
		{
			...
			var site = Sites.findOne({_id : asiteId });
			
			if (site) {
				var result = Meteor.http.post("http://" + Meteor.settings.crawler.host + ":" + Meteor.settings.crawler.port + "/schedule.json", {params:{project:"musiccrawler",spider:"feedspider",setting:"DOWNLOAD_DELAY=2",feedurl:site.feedurl}, timeout: 3000})
				...
				{	
					...
					if (content.status == "error")
						throw new Meteor.Error(500,"unerwarteter Fehler",content.message);
					else return result;
				}
				else throw new Meteor.Error(500,"unerwarteter Fehler","Der Crawl-Lauf konnte nicht erstellt werden.");
			}
			else throw new Meteor.Error(404,"Seite nicht gefunden","Der Crawl-Lauf konnte nicht erstellt werden, da die Seite nicht gefunden wurde.");
		} 
		else throw new Meteor.Error(401,"keine Berechtigung","Der Crawl-Lauf konnte nicht erstellt werden, da du keine Berechtigung dazu hast.");
	},
	...
	cancelCrawl : function(asiteId) {
		...
		if (Meteor.user().admin === true)
		{
			...
			if (site) {
				var result = Meteor.http.post("http://" + Meteor.settings.crawler.host + ":" + Meteor.settings.crawler.port + "/cancel.json", {params:{project:"musiccrawler",job:site.next_crawl}, timeout: 3000})
				...
				else throw new Meteor.Error(500,"unerwarteter Fehler","Der Crawl-Lauf konnte nicht gestoppt werden.");
			}
			else throw new Meteor.Error(404,"Seite nicht gefunden","Der Crawl-Lauf konnte nicht gestoppt werden, da die Seite nicht gefunden wurde.");
		}
		else throw new Meteor.Error(401,"keine Berechtigung","Der Crawl-Lauf konnte nicht gestoppt werden, da du keine Berechtigung dazu hast.");
	}
});